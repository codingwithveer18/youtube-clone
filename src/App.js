...
function App() {
  ...
  const [product, dispatches] = useReducer(videoReducers, []);
  const [mode, setmode] = useState("lightmode");
  const fallbackProducts = ProductsDB; // Fallback data from ProductsDB
  ...
  //   }
  // }
  function videoReducers(product, actions) {
    switch (actions.type) {
      case "LOAD":
        return actions.payload;
      case "ADD":
        return [...product, { ...actions.payload, id: product.length + 1 }];
      case "DELETE":
        return product.filter((product) => product.id !== actions.payload);
      case "UPDATE":
        const index = product.findIndex((v) => v.id === actions.payload.id);
        const updvid = [...product];
        updvid.splice(index, 1, actions.payload);
        setEditableVideo(null);
        return updvid;
      default:
        return product;
  ...
  function editVideo(id) {
    setEditableVideo(product.find((video) => video.id === id));
  }
